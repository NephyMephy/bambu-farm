...
API_BIND="0.0.0.0:${PORT}" cargo run &>/tmp/bambu-api.log &
SERVER_PID=$!
...
echo ""
echo "=== Get printer ==="
curl -s "${BASE}/v1/printers/test-p1" | jq .

echo ""
echo "=== Stream URL (stopped) ==="
curl -s "${BASE}/v1/printers/test-p1/stream/url" | jq .

echo ""
echo "=== Stop stream (not running) ==="
curl -s -X POST "${BASE}/v1/printers/test-p1/stream/stop" | jq .
...
CODE=$(curl -s -o /dev/null -w "%{http_code}" "${BASE}/v1/printers/nope")
echo "Status: ${CODE}"
...
echo ""
echo "=== Delete printer ==="
CODE=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "${BASE}/v1/printers/test-p1")
echo "Status: ${CODE}"
...
CODE=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "${BASE}/v1/printers/nope")
echo "Status: ${CODE}"
