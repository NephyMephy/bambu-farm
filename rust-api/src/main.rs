...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/api/v2/jobs/{id}/complete", post(job_endpoints::complete_job))
        .route("/api/v2/jobs/{id}/status", put(job_endpoints::set_job_status))
        .route("/api/v2/jobs/{id}/printer", put(job_endpoints::set_job_printer))
        .route("/api/v2/jobs/{id}", get(job_endpoints::get_job).delete(job_endpoints::delete_job))
    ...
    let listener = tokio::net::TcpListener::bind(&bind_addr)
        .await
        .unwrap_or_else(|e| panic!("failed to bind API listener ({}): {}", bind_addr, e));
